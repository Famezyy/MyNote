...
一款高性能的 7 层负载均衡软件，支持 TCP/HTTP 两种协议，基于事件驱动的链接处理模式及单进程处理模式使其性能和 Nginx 相当；拥有一个功能出色的监控页面；提供功能强大的 ACL（访问控制列表）。单纯从效率上来讲 HAProxy 更会比 Nginx 有更出色的负载均衡速度，在并发处理上也是优于 Nginx 的。
