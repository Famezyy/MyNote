...
> - 生成的是`被代理类的子类`，因此`final`修饰的类不能被 Cglib 代理
...
![image-20220205001630390](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220205001630390.png)
        ...
        enhancer.setCallback(new MethodInterceptor() {
            ...
            public Object intercept(Object o, Method method, Object[] objects, MethodProxy methodProxy) throws Throwable {
                if (method.getName().equals("add")) {
                    ...
                    // methodProxy.invoke(userService, objects); // 执行 add 方法，传入 o 会陷入循环调用
...
```java
UserService$$EnhancerByCGLIB$$192bd503.class
...
> 使用 MethodProxy 会额外产生两个类：
>
> `UserService$$EnhancerByCGLIB$$192bd503$$FastClassByCGLIB$$f3ed1fe2`：UserService 代理类的 FastClass 代理类
>
> `UserService$$FastClassByCGLIB$$86043490`：UserService 的 FastClass 代理类
>
> 实际执行方法时没有通过反射，所以==速度更快==
