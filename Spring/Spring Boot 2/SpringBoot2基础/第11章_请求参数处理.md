...
上面例子中，`@InitBinder`方法会帮助我们把String类型的参数先trim再绑定，而对于Date类型的参数会先格式化在绑定。例如当请求是`/test?name=%20zero%20&date=2018-05-22`时，会把 zero 绑定到 name，再把时间串格式化为 Date 类型，再绑定到 date。

这里的`@InitBinder`方法只对当前Controller生效，要想全局生效，可以使用`@ControllerAdvice`。通过`@ControllerAdvice`可以将对于控制器的全局配置放置在同一个位置。
