...
```java
new BootStrap().option() // 给客户端的 SocketChannel 配置参数
new ServerBootStrap().option(); // 给服务端 ServerSocketChannel 配置参数
new ServerBootStrap().childOption(); // 给服务端 SocketChannel 配置参数
```
...
netty 中可以通过`option(ChannelOption.SO_BACKLOG，值)`来设置**全连接队列**大小。
...
public class DefaultServerSocketChannelConfig extends DefaultChannelConfig
    ...
    // windows -> 200；linux -> 128
    // 查找 /proc/sys/net/core/somaxconn 文件是否存在，若存在则覆盖值
}
```
...
* 属于操作系统参数，控制一个进程能打开的最大的文件描述符（FD）数量

#### 4）TCP_NODELAY

* 属于`SocketChannal`参数，默认 false，开启 nagle 算法，及为了最大化利用网络资源，会收集到多个小数据包后再发送，会造成发送延迟

#### 5）SO_SNDBUF & SO_RCVBUF（操作系统会动态调整）
...
* 属于`SocketChannal`参数，在 DefaultChannelConfig 中配置了默认值
* 用来分配 ByteBuf，控制池化非池化，调用`ctx.alloc()`时使用
* 安卓环境默认 unpooled，其他环境默认 pooled
...
> 对于网络读取的数据，强制使用直接内存，池化非池化可配置

#### 7）RCBUF_ALLOCATOR

* 属于`SocketChannal`参数，用来控制 netty ByteBuf 大小和是否使用直接内存
...
public class RpcRequestMessageHandler extends SimpleChannelInboundHandler<RpcRequestMessage> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, RpcRequestMessage message) {
        ...
        try {
            ...
            e.printStackTrace();
            // 调用异常
            //  e 包含了所有堆栈信息，直接返回数据太大，可以自己 new 一个异常
...
public class RpcClientManager {
    ...
    public static <T> T getProxyService(Class<T> serviceClass) {
        ...
        Object o = Proxy.newProxyInstance(loader, interfaces, (proxy, method, args) -> {
            ...
            // 4. 等待 promise 结果，用 sync 的话会抛出异常
            promise.await();
            if(promise.isSuccess()) {
                // 调用正常
...
关键代码`io.netty.bootstrap.AbstractBootstrap#initAndRegister` -- 1.

```java
final ChannelFuture initAndRegister() {
    Channel channel = null;
    try {
        // 反射创建 NioServerSocketChannel，在其构造方法中调用 provider.openServerSocketChannel() 创建原生的 ServerSocketChannel
...
关键代码`io.netty.bootstrap.ServerBootstrap#init` -- 1.1
...
关键代码`io.netty.channel.AbstractChannel.AbstractUnsafe#register` -- 1.2
...
关键代码`io.netty.channel.socket.nio.NioServerSocketChannel#doBind` -- 3.3
...
关键代码`io.netty.channel.DefaultChannelPipeline.HeadContext#channelActive` -- 3.4
...
NioEventLoop 线程不仅处理 IO 事件，还利用任务队列处理 Task（包括普通任务和定时任务）
    ...
    for (;;) {
        try {
            try {
                ...
                // 有任务，会执行一次 selectNow，顺便拿到 io 事件，清除上一次的 wakeup 结果，无论有没有 IO 事件，都会跳过 switch
            ...
            // ioRatio 用来控制处理 io 事件所占用的时间比例，默认是 50
...
> * `wakenUp.compareAndSet(false, true)`用于控制多线程下只有一个线程可以调用 wakeup 方法（调用一次就可以唤醒了）

参考下图

<img src="https://raw.githubusercontent.com/Famezyy/picture/master/notePictureBed/0032-ed4d303765160098439016dfb1de519d-2e07e9.png" style="zoom:80%;" />
...
- selector 在 NioEventLoop 的构造方法调用时创建，首先创建了原生的 selector（unwrappedSelector），但是由于原生的 selector 使用了 set 来存储 selectionKeys，其遍历效率不高，所以 netty 又封装了一个 selector，内部用数组存储 selectionKeys
- eventLoop 的 nio 线程在首次调用 execute 方法时被创建，通过 state 状态位控制线程只会启动一次
- 提交普通任务会调用 wakeup 方法唤醒`selector.select()`

### 2.3 accept剖析

- `selector.select()`阻塞直到事件发生
- 遍历处理 selectionKeys
- 拿到一个 key，判断是否是 accept
- 创建 SocketChannel，设置非阻塞，并创建 NioSocketChannel
- 将 SocketChannel 注册到 selector，`sc.register(eventLoop的选择器, 0, NioSocketChannel)`
