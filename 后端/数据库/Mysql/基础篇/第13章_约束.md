...
可以在**创建表时规定约束（通过 CREATE TABLE 语句）**，或者在**表创建之后通过 ALTER TABLE 语句增加或删除约束**。
...
> 注意，若表中存在对应字段为 null 的数据，则不能修改该字段为非空约束
...
ALTER TABLE MODIFY 表名称 PRIMARY KEY;
```
...
（3）自增长列约束的列必须是键列（主键列，唯一键列），==要注意保证自增长列在删除键列后至少满足主键列或唯一键列其中之一！==

（4）自增约束的列的数据类型必须是整数类型

（5）如果自增列指定了 **0** 和 **null**，会在当前最大值的基础上自增；如果自增列手动指定了具体值（包括负数），直接赋值为具体值
...
​    在 MySQL 8.0 之前，自增主键 AUTO_INCREMENT 的值如果大于 max(primary key)+1，在 MySQL 重启后，会重置 AUTO_INCREMENT=max(primary key)+1，这种现象在某些情况下会导致业务主键冲突或者其他难以发现的问题。下面通过案例来对比不同的版本中自增变量是否持久化。
...
（1）从表的外键列，必须引用/参考主表的==主键==或==唯一约束==的列
...
#ERROR 1215 (HY000): Cannot add foreign key constraint  原因是dept的did不是键列，即没有主键约束或唯一性约束
```
