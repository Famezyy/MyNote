...
```mysql
SELECT ROUND(12.33),ROUND(12.343,2),ROUND(12.324,-1) /* -1表示从个位数开始四舍五入 */,TRUNCATE(12.66,1),TRUNCATE(12.66,-1)
FROM DUAL;
```
...
![image-20220108204845285](C:\Users\Administrator\AppData\Roaming\Typora\typora-user-images\image-20220108204845285.png)
...
```mysql
SELECT CURDATE(),CURTIME(),NOW(),SYSDATE()+0,UTC_DATE(),UTC_DATE()+0,UTC_TIME(),UTC_TIME()+0 # 会自动隐式转换
FROM DUAL;
```
...
| DAYOFWEEK(date)                          | 返回这一周的第几天，注意：周日是第1天，周一是第2天，。。。周六是第7天 |
...
```mysql
SELECT MONTHNAME('2021-10-26'),DAYNAME('2021-10-26'),WEEKDAY('2021-10-26'), # 传入字符串会隐式转换为 date 类型
...
| 函数                        | 用法                                                         |
| --------------------------- | ------------------------------------------------------------ |
| PASSWORD(str)               | 返回字符串str的加密版本，41位长的字符串。加密结果`不可逆`，常用于用户的密码加密，MySQL8.0中不再提供 |
...
| 函数                           | 用法                                                         |
| ------------------------------ | ------------------------------------------------------------ |
| FORMAT(value,n)                | 返回对数字value进行格式化后的结果数据。n表示`四舍五入`后保留到小数点后n位，小于零时只保存整数位，不同于 [round](#2.1 基本函数) |
