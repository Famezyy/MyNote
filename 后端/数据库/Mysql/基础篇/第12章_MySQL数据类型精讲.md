...
| UNSIGNED           | 无符号                                                   |
| CHARACTER SET name | 指定一个字符集（创建数据库，创建表，创建字段时均可使用） |
...
使用SELECT命令查询位字段时，可以用`BIN()`或`HEX()`（16进制，默认）函数进行读取。
...
可以看到，使用 存储二进制的列名+0 查询数据时，可以直接查询出存储的十进制数据的值。
...
- 使用`CURRENT_DATE()`，`CURDATE()`或者`NOW()`函数，会插入当前系统的日期。
...
（3）使用`CURRENT_TIME()`，`CURTIME()`或者`NOW()`，会插入当前系统的时间。
...
- 底层存储方式不同，TIMESTAMP底层存储的是**毫秒值**，距离1970-1-1 0:0:0 0毫秒的毫秒值。
...
| ---------------- | -------- | ---- | ----------------------- | ------------------------------------------------------------ |
| CHAR(M)          | 固定长度 | M    | 0 <= M <= 255（字节）   | M个字节                                                      |
| VARCHAR(M)       | 可变长度 | M    | 0 <= M <= 65535（字节） | ==最大指定65532个字节用户数据长度+2个字节的长度标识+1个字节的 null 值标识（无 NOT NULL时）==<br />当字节较小时，指定多少个字节用户就可以使用多少个，长度表示和 null 标识会额外占用空间 |

> MySQL  规定除了 TEXT 和 BLOB 外的所有列的长度和小于 65535 字节（不包括隐藏列和记录头信息）。
...
- MySQL4.0版本以下，varchar(20)：指的是20字节，如果存放UTF8汉字时，只能存6个（每个汉字3字节） ；**MySQL5.0版本以上，varchar(20)：指的是20字符**，所以varchar最大能指定(65535/3)
...
# 允许按照角标的方式获取指定索引位置的枚举值，支持数值和字符串
INSERT INTO test_enum
VALUES('1'),(3);
