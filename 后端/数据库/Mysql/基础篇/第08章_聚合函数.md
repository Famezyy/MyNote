...
> **去重场景下，DSTINCT 与 GROUP BY 的不同**
>
> - 有索引时
>
>   两者都会进行分组且都能使用索引，效率相同，`DINSCT`可以看作是特殊的`GROUP BY`，但是只能用于去重，只会显示去重后的列，没有其他列的信息。而`GROUP BY`可以针对指定列进行分组，并显示其他列的信息。
>
> - 无索引时
>
>   两者都会进行分组，但是`GROUP BY`在 MYSQL 8.0 之前会进行隐式排序，触发 filesort，SQL 执行效率较差。原理是`GROUP BY`先对结果进行分组排序（按照`GROUP BY`指定字段），然后返回每组中的第一条数据。可以通过添加`ORDER BY NULL`来禁止隐式排序。
>
>   由于效率原因，在 MYSQL 8.0 中移除了`GROUP BY`的隐式排序。
