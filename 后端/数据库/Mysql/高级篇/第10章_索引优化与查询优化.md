...
​    首先将 index key 条件满足的索引记录区间确定，然后在索引上使用 index filter 进行过滤。将满足的 index filter 条件的索引记录查找后才去回表取出整行记录返回 server 层。不满足 index filter 条件的索引记录丢弃，不回表、也不会返回 server 层。
...
​    使用前，存储层多返回了需要被 index filter 过滤掉的整行记录，使用 ICP 后，直接就去掉了不满足 index filter 条件的记录，**省去了他们回表和传递到 server 层的成本**。ICP 的加速效果取决于在存储引擎内通过 ICP 筛选掉的数据的比例。
