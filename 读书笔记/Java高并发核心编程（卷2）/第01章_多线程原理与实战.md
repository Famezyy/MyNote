...
创建一个“可缓存线程池”，如果线程池内的某些线程空闲，则可以被灵活回收。该线程池特点如下：
...
> **注意**
>
> 以上这 4 个创建线程池的方法都存在一个弊端：其内部使用无界队列存放排队任务，当大量任务超过最大容量时队列会无限增大，使服务器资源迅速耗尽。
...
执行 `shutdown()` 方法后，线程池状态会变为 SHUTDOWN，此时线程池将拒绝新任务，不能再往线程池中添加任务，否则会抛出 `RejectedExecutionException` 异常。此外线程池不会立即退出，直到**添加到线程池中的任务都已经处理完成**才会退出。还有一个与 `shutdown()` 类似的方法，叫做 `shutdownNow()`，执行该方法后线程池状态会立刻变成 STOP，并试图停止所有正在执行的线程，并且不再处理还在阻塞队列中等待的任务，会返回那些未执行的任务。
