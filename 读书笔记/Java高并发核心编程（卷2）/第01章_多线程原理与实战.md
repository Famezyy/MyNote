...
Java 高并发应用频繁创建和销毁线程的操作非常低效，而且是**不被编程规范所允许**的，因此必须使用线程池。线程池主要有以下三个好处：

- **降低资源消耗**

  线程是稀缺资源，如果无限制地创建，不仅会消耗系统资源，还会降低系统的稳定性，通过重复利用已创建的线程可以降低线程创建和销毁的成本。

- **提高响应速度**

  当任务到达时，不需要等待线程创建就能立即执行。
  
- **提高线程的可管理性**

  线程池提供了一种限制、管理资源的策略，维护了一些基本的线程统计信息，如已完成的任务数量等。通过线程池可以对线程资源进行统一的分配、监控和调优。
...
- 调用 `submit()` 方法
  ...
  <T> Future<T> submit(Callable<T> task);
  <T> Future<T> submit(Runnable task, T result);         // 调用 get 返回 result
  Future<?> submit(Runnable task);　　　　　　　　　　　　　// 调用 get 会返回 null
  ```
...
- **`submit()` 可以进行 `Exception` 处理**
  ...
  > **注意**
  >
  > - 抛出异常时不会影响线程池里面其他线程的正常执行
  >
  > - `execute` 执行中如果发生异常，会直接抛出堆栈异常，然后线程池会把该线程移除掉，并创建一个新的线程放到线程池中
  > - `submit` 会将传进来的任务封装成 `FutureTask`，如果抛出异常，并不会 throw x，而是 `setException()` 保存异常，当我们阻塞获取 `submit()` 方法结果时 `get()` 才会将异常信息抛出；因为线程池的 `runWorker()` 中没有抛出异常，所以并不会删除线程
...
使用 `newSingleThreadExecutor` 的问题同样在于使用的是**==无界队列==**，如果任务提交速度大于处理速度，就会造成队列容量急剧增大，最后导致 OOM 异常。
...
这里创建了一个核心线程数为 0，最大线程数不设限制的线程池，理论上可以有无数个工作线程。“可缓存线程池”使用的是 `SynchronousQueue` 同步队列，这个队列类似于一个接力棒，入队出队必须同时传递。正因为可以无限制创建线程，不会有任务等待，所以才使用 `SynchronousQueue`。每次新任务会被放入同步队列，如果当前有可用线程则执行，如果没有则创建一个新的线程来执行，因此 `maximumPoolSize` 需要设置的相当大，从而使得新任务不会被拒绝。

使用 `newCachedThreadPool` 的主要问题在于**==最大线程数量没有上限==**，可以无限创建线程，如果任务提交过多，会造成 OOM 异常，甚至导致 CPU 线程资源耗尽。
...
// ScheduledThreadPoolExecutor 继承了 ThreadPoolExecutor
...
问题同样在于**==最大线程数量没有上限==**，并且使用的是**==无界队列==**，任务数量过多会导致 CPU 资源耗尽和 OOM 异常。

### 5.9 确定线程池线程数

使用 `ThreadPoolExecutor` 创建线程池时，线程数的配置与异步任务类型是分不开的。线程池的异步任务大致分为以下三类：

- **IO 密集型任务**

  此类任务主要是执行 IO 操作。由于执行 IO 操作的时间较长，导致 CPU 利用率不高，这类任务 CPU 常处于空闲状态。例如 Netty 的 IO 读写操作。
...
- **混合型任务**
  ...
  此类任务既要执行逻辑计算，又要进行 IO 操作（如 RPC 调用、数据库访问）。相对来说，由于执行 IO 操作的耗时较长（一次网络请求往往百毫秒），这类任务的 CPU 利用率也不是太高。

#### 1.IO密集型任务线程数

由于 IO 密集型任务 CPU 使用率较低，导致线程空余时间很多，因此通常需要开 PCU 核心数 2 倍的线程。当 IO 线程空闲时，可以启用其他线程继续使用 CPU。Netty 的 IO 处理任务就是典型的 IO 密集型任务。所以 Netty 的 Reactor 实现类的 IO 处理线程数默认是 CPU 核数的 2 倍。
...
- 调用线程池的 `allowCoreThreadTimeOut(true)` 方法使得 `keepAliveTime` 参数同样作用于核心线程，当核心线程数长时间空闲时可以自行销毁
- 使用有界缓冲队列
- `corePoolSize` 和 `maximumPoolSize` 保持一致，避免动态创建删除线程带来的消耗；同时由于设置了核心线程数空闲销毁，当新任务进来且没有空闲线程时，会优先创建新任务而不是放入阻塞队列
- 使用 JVM 关闭时的钩子函数关闭线程池

#### 2.CPU密集型任务线程数

CPU 密集型任务也叫计算密集型任务，特点是要进行大量计算消耗 CPU 资源，如计算圆周率，对视频进行解码等。CPU 密集型任务虽然也可以并行完成，但是并行的任务越多，花在任务切换的时间就越多，CPU 执行任务的效率就越低，所以此时并行执行的数量应当**等于 CPU 核心数**。

#### 3.混合型任务线程数

混合型任务既要执行逻辑计算，也要进行大量非 CPU 耗时操作（如 RPC 调用、数据库访问、网络通信等），所以混合型任务 CPU 的利用率不是太高，非 CPU 耗时往往是 CPU 耗时的数倍。比如在 Web 应用中处理 HTTP 请求时，一次请求处理会包括 DB 操作、RPC 操作、缓存操作等多种耗时操作。一般来说，一次 Web 请求的 CPU 计算耗时往往较少，大致在 100～500 毫秒，而其他耗时操作会占用 500～1000 毫秒。

在计算混合型任务线程池线程数时有一个比较成熟的估算公式：

```bash
最佳线程数 =（（线程等待时间 + 线程 CPU 时间）/ 线程 CPU 时间） * CPU 核心数
```

可以进一步转换为：

```bash
最佳线程数 =（线程等待时间与线程 CPU 时间之比 + 1） * CPU 核心数
```

可以看出：等待时间所占比例越高，需要的线程就越多；CPU 耗时所占的比例越高，需要的线程就越少。

> **注意**
>
> 公式只是理论最佳值，生产环境中需要结合系统网络环境和硬件情况不断尝试，获取一个符合实际的线程数值。

实践中可以通过读取启动变量或环境变量来设置线程数：

```java
System.getProperty(“mixed.thread.amount”);
System.getEnv(“mixed.thread.amount”);
```
